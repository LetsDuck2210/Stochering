...
public class Main {
			...
			try {
				...
				if (out.getClass().isArray()) {
					...
					for (int i = 0; i < len; i++) {
						...
						|| out instanceof BigDecimal && Main.<Integer>getOption("output_accuracy") != -1) {
					System.out.println(String.format("%." + getOption("output_accuracy") + "f", out).replaceAll("0*$", "")
							.replaceAll("\\.$", ""));
				} else
	...
	public static Object invokeFunction(String fuzzyName, String[] params)
